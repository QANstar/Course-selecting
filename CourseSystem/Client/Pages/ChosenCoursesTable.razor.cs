...
{
    public partial class ChosenCoursesTable
    {
        bool isLoading = true;
        ...
        protected async override Task OnInitializedAsync()
        {
            ...
            if(studentsCourseInfoData[0].ChooseCourse.Replace(" ", "") == "")
            {
                getData = null;
            ...
            {
                getData = await Http.GetFromJsonAsync<List<CourseInfoData>>("api/courseinfo/getcourseinfo?courseId=" + studentsCourseInfoData[0].ChooseCourse);
        ...
        private async Task OnRowButtonClick(CourseInfoData item)
        {
            ...
            var result = await Http.PostAsJsonAsync<StudentsCoursesModel>($"api/StudentInfo/CancelCourse", addCourse);
            var courseResult = await Http.PostAsJsonAsync<CourseInfoData>($"api/CourseInfo/CancelStudent", courseData);
